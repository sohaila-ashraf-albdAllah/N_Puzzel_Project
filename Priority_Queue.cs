...
{
    class Priority_Queue //(Nlog N)
    {
        public List<Puzzel> PUZZLE = new List<Puzzel>();
        public void Enqueue(Puzzel item)//O(Nlog N)
        {
            PUZZLE.Add(item);//O(1)
            int parent_index;//O(1)
            for (int child_index = PUZZLE.Count - 1; child_index > 0; child_index = child_index / 2)//O(log N) * O(N) --> O(Nlog N)
            {
                parent_index = (child_index - 1) / 2;//O(1)
                if ((PUZZLE[child_index].cost) >= (PUZZLE[parent_index].cost))//O(N)
                {
                    break;//O(1)
                }
        ...
        public Puzzel Dequeue()//O(Nlog N)
        {
            Puzzel frontItem = PUZZLE[0];//O(N)
            int lastindex_beforeremove = PUZZLE.Count - 1;//O(N)
            ...
            for (int child_index = PUZZLE.Count ; child_index > 0; child_index = child_index / 2)//O(log N) * O(N) --> O(Nlog N)
            {
                ...
                if (leftchild_index <= lastindex_afterchange)//O(1) * O(N) --> O(N)
                {
                    ...
                    if (PUZZLE[parent_index].cost <= (PUZZLE[leftchild_index].cost))//O(N)
                    {
                        break;//O(1)
                    }
                    Puzzel tmp = PUZZLE[parent_index];//O(N)
