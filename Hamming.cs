...
{        
    class Hamming //O(N^7)
    {
        ...
        //***********************************************************************************
        public void Get_final_Path(Puzzel final) //O(N^2)
        {
        ...
        //***********************************************************************************
        public void Display_(int size) //O(N^2)
        {
        ...
        //***********************************************************************************
        public void Create_New_Child(Puzzel p) //O(N^3)
        {
            if (p.check_Movement_value(1,0,0,0)==true) //O(N) * O(N^2) --> O(N^3)
            {
                Puzzel child = new Puzzel(p); //O(N^3)
                bool check; //O(1)
                child.UP_movement(); //O(N)
                child.Hamming(); //O(N)
                child.Calculate_cost_of_hamming(); //O(1)

                if (child.IS_reache_to_goal_hamming() == true) //O(1) * O(N^2) --> O(N^2)
                {
                ...
                check = Child_Open(child); //O(N ^ 2)

                if (check == false) //O(1) * O(N^2) --> O(N^2)
                {
                    PQ_list.Enqueue(child);//O(N^2)
                    Open_child.Add(child.key, child); //O(1)
                }
            }

            if (p.check_Movement_value(0, 1, 0, 0) == true)//O(N) * O(N^2) --> O(N^3)
            {
                Puzzel child = new Puzzel(p);//O(N^3)
                bool check;//O(1)
            ...
            if (p.check_Movement_value(0, 0, 1, 0) == true)//O(N) * O(N^2) --> O(N^3)
            {
                Puzzel child = new Puzzel(p);//O(N^3)
                bool check;//O(1)
            ...
            if (p.check_Movement_value(0, 0, 0, 1) == true) //O(N) * O(N ^ 2)-- > O(N ^ 3)
            {
                Puzzel child = new Puzzel(p);//O(N^3)
                bool check;//O(1)
        ...
        //***********************************************************************************
        public void A_Star_Algorithm_wiht_hamming(Puzzel First)//O(N^7)
        {
